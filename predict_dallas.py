...
def predict(path):
    ...
    for seq in sample:
        ...
        seq['times'] = [x*(831/seq['times'][-1]) for x in seq['times']]
        seq['times']=np.floor(seq['times'])
        for i in range(1,len(seq['times'])):
            if seq['times'][i] in range(801,811,1):
                result1.append(seq['nodes'][i])
            elif seq['times'][i]in range(811,821,1):
                result2.append(seq['nodes'][i])
            elif seq['times'][i]in range(821,831,1):
...
def groundtruth():
    data = np.loadtxt("data/dallas/Dallas.txt")
    cnt=list()
    data1=data[3130:3217]
    cnt1 = Counter(data1[:, 0].flatten())
    cnt.append(cnt1)
    data2 = data[3217:3322]
    cnt2 = Counter(data2[:, 0].flatten())
    cnt.append(cnt2)
    data3 = data[3322:3371]
...
def rank(r,cnt,top):
    ...
    for i in range(0,len(r)):
        ...
        t.append(truth)
        a.append(all)
    np.savetxt("predict/dallas_pre_all_{}.txt".format(top), a, fmt='%s')
    np.savetxt("predict/dallas_pre_{}.txt".format(top), p,fmt='%s')
    np.save("predict/dallas_truth.npy",t)
def MAP_cal(para):
    d1 = np.loadtxt("predict/dallas_pre_{}.txt".format(para))
    d2=np.load("predict/dallas_truth.npy",allow_pickle=True)
    MAP = 0
    for i in range(0,3):
        map_sub = 0
        c = 0
...
r=predict(path='model_mle_dallas.pkl')
cnt=groundtruth()
para_list=[3,5,7,10]
for para in para_list:
    ...
    print("mmr:")
    print(z)
    with open("predict/prediction_dallas_4.txt", 'a') as l:
